...
VertexShaderOutput VertexShaderFunction(VertexShaderInput input)
{
    ...
    output.View = CameraPos - VertexPosition;
...
float4 PixelShaderFunction(VertexShaderOutput input) : COLOR0
{
	...
	float3 reflection = reflect(-normalize(input.View), normalize(input.Normal));
    ...
    return (saturate(textureColor * (diffuseIntensity) + AmbientColor * AmbientIntensity + specular)) * texCUBE(ReflectiveSampler, normalize(reflection));
}
